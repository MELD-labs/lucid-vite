...
import { applyDoubleCborEncoding, nativeScriptFromJson, fromHex, toHex } from "../utils/mod.js";
            ...
            scriptRef: r.reference_script_hash
                ? (await (async () => {
                    ...
                    if (type === "Native" || type === "native" || type === "timelock") {
                        const { json: script } = await fetch(`${this.url}/scripts/${r.reference_script_hash}/json`, { headers: { project_id: this.projectId, lucid } }).then((res) => res.json());
